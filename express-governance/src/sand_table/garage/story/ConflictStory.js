const {
    ...
    LIFTER_STATUS_AUTO,
    LIFTER_STATUS_HAND, } = require("../const")
const { RegularStory } = require("./RegularStory");
const { generateCarLicenseTag } = require("../utils");
...
class ConflictStory {
    constructor(sandTable) {
        ...
        this.detectList = sandTable.detectList
        this.regularStory = new RegularStory(sandTable)


        // 0-矛盾未解决，1-矛盾解决，2-矛盾完全解决
        this.conflictStatus = 0;

        // 活动车辆停车时间
        this.activeCarStopTime = 5
        ...
        // 发生冲突的时间
        this.conflictSteps = [39, 38]

        this.conflictPosition = [-36.7, 0.1, -4.2]
        this.conflictInfoId = uuid.v1()


        this.garageCurrentInfo = {
            lifters: this.lifterList,
            // 初始化常驻车辆信息
            cars: [...this.regularStory.fixedCarInfoList],
            lights: this.lightList,
            detects: this.detectList
        }
        this.initConflictStory();
    }

    initConflictStory() {
        this.askTimes = 0
        this.activeCarList.push(...(_.cloneDeep(this.activeCarInfoList)))
    ...
    resetStory() {
        return this.regularStory.resetStory()
    }

    getMessage() {
        if (this.conflictStatus !== 2) {
            const result = this.getAccidentMessage();
            if (result) {
                return result;
            }
        }
        return this.regularStory.getMessage()
    }

    getAccidentMessage() {
        // 冲突车辆完全离开车库
        if (this.checkIfAllCarLeave() && this.conflictStatus === 1) {
            this.conflictStatus = 2;
            return
        }
        ...
        this.activeCarList.forEach(
            (activeCar, index) => {
                ...
                const conflictStep = this.conflictSteps[index]
                activeCar.carInfo.alarmType = ALARM_TYPE_NONE

                if (activeCarStep === conflictStep && this.conflictStatus === 0) {
                    this.sandTable.addDetectInfo({
                        id: this.conflictInfoId,
                        type: DETECT_TYPE_CONFLICT,
                        content: "监测到车辆碰撞",
                        cars: this.activeCarList.map(car => car.carInfo),
                        positionArray: this.conflictPosition
                    })
                    return;
                }
                ...
                if (activeCarStep >= 0) {
                    ...
                    if (flag) {
                        ...
                        if (activeCar.carEnterTime <= activeCarStep && activeCarStep < activeCar.carStopTime) {
                            ...
                            activeCar.carInfo.parkingStatus = PARKING_STATUS_ENTER
                        ...
                        if (activeCar.carStopTime <= activeCarStep && activeCarStep < activeCar.carLeaveTime) {
                            activeCar.carInfo.parkingStatus = PARKING_STATUS_STOP
                        ...
                        if (activeCar.carLeaveTime <= activeCarStep && activeCarStep < activeCar.carOutTime) {
                            ...
                            currentPositionInfo.timeStamp = new Date().getTime()
                            activeCar.carInfo.parkingStatus = PARKING_STATUS_LEAVE
                        ...
                        activeCar.carInfo.parkingStatus = PARKING_STATUS_ANOMALY
                        activeCar.carInfo.alarmType = ALARM_TYPE_WHISTLE
    ...
    endConflict() {
        ...
        this.sandTable.removeDetectInfo(this.conflictInfoId)
    }
}

module.exports = { ConflictStory };