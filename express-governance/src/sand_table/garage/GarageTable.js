...
const { ConflictStory } = require("./story/ConflictStory")
...
const gateMonitorList = [
    {
        id: 1,
        name: '南门-出口',
        targetPosition: [11.674, 1.2, -27.968],
        cameraPosition: [10.027, 7.160, -38.576],
        monitorUrl: '/monitor/garage/garage_south_enter_light.mp4'
...
class GarageTable {
    constructor() {
        ...
        this.gateMonitorList = gateMonitorList;
        this.detectList = [];
        this.story = new RegularStory(this);
        this.story = new NightStory(this)
        // this.story = new ConflictStory(this);
    }
    ...
    sendGarageMessage() {
        const message = this.getGarageMessage();
        // 遍历连接池发送信息
        this.wsList.forEach(ws => {
            ws.send(JSON.stringify(message));
        })
    ...
    getMessage() {
        return { tag: "garage_status", data: this.story.getMessage() }
    ...
    getGarageMessage() {
        let cars = []
        if (this.message) {
            cars = this.message.data.cars;
        }
        return { tag: "garage_status", data: { lifters: this.lifterList, lights: this.lightList, detects: this.detectList, cars } }
    ...
    upLifter(id) {
        id = parseInt(id)
        const lifter = this.lifterList.find(lifter => lifter.id === id)
        if (lifter) {
            lifter.isRaised = true;
        }
        return this.lifterList[0].id === id
    }
    ...
    changeLifterStatus(id, status) {
        const lifter = this.lifterList.find(lifter => lifter.id === id && lifter.status !== LIFTER_STATUS_ANOMALY)
        if (lifter) {
    ...
    openLight(id) {
        id = parseInt(id)
        const light = this.lightList.find(light => light.id === id)
        if (light) {
            light.isOpen = true;
        }
    }
    ...
    addDetectInfo(info) {
        let index = this.detectList.findIndex(item => item.id === info.id)
        if (index !== -1) {
            this.detectList.splice(index, 1, info);
        } else {
            this.detectList.push(info);
        }
    }

    removeDetectInfo(id) {
        const index = this.detectList.findIndex(item => item.id !== id)
        if (index !== -1) {
    ...
    endConflict(id) {
        if (this.story instanceof ConflictStory) {
            this.story.endConflict(id);
        }
    }
