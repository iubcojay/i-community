...
class LifterFaultStory {
    constructor(sandTable) {
        ...
        // console.log(pathCollection[1].enter.length)
        ...
        this.activeCarInfoList = [{
            carEnterTime: 0,
            carOutTime: pathCollection[5].leave.length,
            pathTemplate: pathCollection[5],
            lifterInfoList: [ { lifterId: 1, raisedTimes: [ 2,3,4] }],
            targetParkingSpaceNumber: 'A034',
            carInfo: {
                id: uuid.v1(),
                type: '',
            ...
            // console.log(this.activeCarInfoList[0].pathTemplate)
        ...
        this.lifterList.forEach(lifter => {
            if (lifter.status === LIFTER_TYPE.LIFTER_STATUS_AUTO) {
                ...
                this.sandTable.changeLifterStatus(1, 1)
            }
    ...
    resetStory(a) {
        ...
        else if(a === 2 && this.repeatTimes !== 0){
            this.activeCarList = _.cloneDeep(this.activeCarInfoListRegular)
        }
        ...
        // console.log(this.activeCarSteps)
    }
    ...
    getMessage() {
        ...
        if (this.askTimes === 0 && this.repeatTimes !== 0) {
        ...
        this.activeCarList.forEach(
            (activeCar, index) => {
                ...
                // console.log("Che liang shi fou zai che ku nei ")
                // console.log(activeCarStep)
                // 车辆在库内
                if (activeCarStep >= 0) {
                    // 车辆被道闸阻拦的标志
                    let flag = true
                    ...
                    if (flag) {
                        ...
                        // 车辆入库
                        if (activeCar.carEnterTime <= activeCarStep && activeCarStep < activeCar.carOutTime) {
                            const currentPositionInfo = _.cloneDeep(activeCar.pathTemplate.leave[activeCarStep])
    ...
    repeatTimesCounter() {
        if (this.checkIfAllCarLeave()) {
            ...
            //     let x = this.activeCarInfoList
            //     this.activeCarInfoList = this.activeCarInfoListRegular
            //     this.activeCarInfoListRegular = x
            // }
        }
    }
...
module.exports = {  LifterFaultStory };