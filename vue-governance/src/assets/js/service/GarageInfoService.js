...
import { RegularStroy, LifterBrokenStory, LifterBrokenLightCloseStory,NightStory } from '@/assets/js/service/GarageStory'
import Garage from '@/assets/js/threejs/Garage'

export default class GarageInfoService {
    constructor() {
        this.lifterList = [
            {
                id: 1,
        ...
        this.regularStroy = new RegularStroy({ lifterList: this.lifterList, lightList: this.lightList })
        // this.lifterBrokenStory = new LifterBrokenStory({ lifterList: this.lifterList, lightList: this.lightList })
        // this.lifterBrokenLightCloseStory = new LifterBrokenLightCloseStory({ lifterList: this.lifterList, lightList: this.lightList })
        this.nightStory = new NightStory({ lifterList: this.lifterList, lightList: this.lightList })
    }

    getNewGarageInfo() {
        let result = null
        if (this.nightStory.repeatTimes === 0) {
           
            result = this.nightStory.getCurrentGarageInfo()

        } else {
            result = this.regularStroy.getCurrentGarageInfo()
        }
        // result = this.regularStroy.getCurrentGarageInfo()
        this.carList = result.car
    ...
    setLifterMode(lifterId, status) {
        const targetLifter = this.lifterList.find(lifter => lifter.id === lifterId)
        targetLifter && (targetLifter.status = status)
    }

    setLifterStatus(lifterId, isRaise) {
        const targetLifter = this.lifterList.find(lifter => lifter.id === lifterId)
        targetLifter && (targetLifter.isRaised = isRaise)
    }
    ...
    toggleLightById(lightId, option) {
        const light = this.lightList.find(item => item.id === lightId)
        if (light) {
            if (!_.isUndefined(option)) {
                light.isOpen = option
            } else {
                light.isOpen = !light.isOpen
            }
        }
    }
}