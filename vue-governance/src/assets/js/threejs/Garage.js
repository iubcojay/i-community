...
export default class Garage {
    ...
    constructor({ gltfLoader = new GLTFLoader() }) {
        this.modelUrl = window.PLATFORM_CONFIG.StaticResourceAddress + '/model/garage/garage.gltf'
    ...
    async initModel() {
        ...
        // this.splitGarage(modelObj)
        this.animations = animations
        // this.modelObj = modelObj
        this.modelObj = modelObj.children[0]
        this.cleanModel(this.modelObj)
        this.addLight()
        this.addBasePlane()
    }
    ...
    addGarageToScene(basicThreeJsScene) {
        if (!this.#ifGarageAdded && this.modelObj) {
            basicThreeJsScene.add(this.modelObj)
            basicThreeJsScene.garage = this
            this.actions = this.initAnimation(this.animations, basicThreeJsScene.animationMixer)
            this.#ifGarageAdded = true
        }
    }
    ...
    // 废除
    splitGarage(garageModel) {
        //清洗模型数据
        getChildByName(garageModel, "rampway").name = 'runway1'
        getChildByName(garageModel, "rampway2").name = 'runway2'
        getChildByName(garageModel, "gate").name = 'gate1'
        getChildByName(garageModel, "ceiling01").name = 'ceiling1'
        getChildByName(garageModel, "celling").name = 'ceiling2'

        // 合并天花板
        const ceilingGroup = new THREE.Group()
        ceilingGroup.name = "ceilingGroup"
        ceilingGroup.add(
            getChildByName(garageModel, 'ceiling1'),
            getChildByName(garageModel, 'ceiling2'),
        )
        garageModel.add(ceilingGroup)
        ceilingGroup.position.y += 1

        const lightGroup = new THREE.Group()
        lightGroup.name = "lightGroup"
        lightGroup.add(
            getChildByName(garageModel, "light01"),
            getChildByName(garageModel, "light02"),
            getChildByName(garageModel, "light03"),
            getChildByName(garageModel, "light04"),
            getChildByName(garageModel, "light05"),
            getChildByName(garageModel, "light06"),
        )
        garageModel.add(lightGroup)


        // 合并跑道
        const runwayGroup = new THREE.Group()
        runwayGroup.name = "runwayGroup"
        runwayGroup.add(getChildByName(garageModel, 'runway1'), getChildByName(garageModel, 'runway2'))
        garageModel.add(runwayGroup)

        // 合并摄像头
        const cameraGroup = new THREE.Group()
        cameraGroup.name = "cameraGroup"
        cameraGroup.add(
            getChildByName(garageModel, 'camera001'),
            getChildByName(garageModel, 'camera002'),
            getChildByName(garageModel, 'camera003'),
            getChildByName(garageModel, 'camera004'),
            getChildByName(garageModel, 'camera005'),
            getChildByName(garageModel, 'camera006'),
        )
        garageModel.add(cameraGroup)

        // 合并大门
        const gateGroup = new THREE.Group()
        gateGroup.name = "gateGroup"
        gateGroup.add(
            getChildByName(garageModel, 'gate1'),
            getChildByName(garageModel, 'gate2'),

        )
        garageModel.add(gateGroup)

        // 合并升降杆
        const lifterGroup = new THREE.Group()
        lifterGroup.name = "lifterGroup"
        lifterGroup.add(
            getChildByName(garageModel, 'lifter1'),
            getChildByName(garageModel, 'lifter2'),
            getChildByName(garageModel, 'lifter01'),
            getChildByName(garageModel, 'lifter02'),
        )
        garageModel.add(lifterGroup)
    ...
    // 升起杆子(仅模型)
    raiseLifterById(id) {
        this.playAnimationByName(this.#lifterIdActionNameDictionary[id].raise)
        this.stopAnimationByName(this.#lifterIdActionNameDictionary[id].descend)
    }

    // 降下杆子(仅模型)
    descendLifterById(id) {
        this.playAnimationByName(this.#lifterIdActionNameDictionary[id].descend)
        this.stopAnimationByName(this.#lifterIdActionNameDictionary[id].raise)
    }
    ...
    clearGarage(basicThreeJsScene) {
        ...
        this.updateIntervalId && clearInterval(this.updateIntervalId)
    }
...
Garage.LIFTER_STATUS_ANOMALY = -1
Garage.LIFTER_STATUS_AUTO = 0
Garage.LIFTER_STATUS_HAND = 1
