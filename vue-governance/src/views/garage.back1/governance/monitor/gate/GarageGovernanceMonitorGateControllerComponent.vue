...
export default {
  ...
  methods: {
    ...
    async getGateMonitorList() {
      const { data } = await this.$axios.get("/garage/monitor");
      const { code, data: result } = data;
      if (code === 200) {
        this.gateMonitorList = result;
      }
      this.gateMonitorList.forEach((monitor) => {
        this.$set(monitor, "ifShow", false);
        monitor.monitorUrl =
          window.PLATFORM_CONFIG.StaticResourceAddress + monitor.monitorUrl;
      });
