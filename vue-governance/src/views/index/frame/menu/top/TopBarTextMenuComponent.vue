<template>
  <div class="text_menu_panel">
    ...
    >
      <div
        class="text_menu_text"
        @click="handleMenuItemClick(menuItem)"
        :class="{ active_item: activeGroup.activeMenuItem === menuItem }"
      >
        <span style="font-size: 20px;">{{ menuItem.text }}</span>
        <i
          v-if="menuItem.subList.length > 0"
      ...
      <div class="text_menu_sub_item_panel">
        <div
          class="text_menu_sub_item"
          v-for="subItem of menuItem.subList"
          :key="subItem.text"
          @click="handleSubItemClick(menuItem, subItem)"
        >
          <div
            style="white-space: nowrap;"
            :class="{ active_item: activeGroup.activeSubItem === subItem }"
          >
            {{ subItem.text }}
          </div>
        </div>
...
export default {
  ...
  methods: {
    initActiveItemMenu() {
      ...
      this.menuData.forEach((menuItem) => {
        ...
        menuItem.subList.forEach((subItem) => {
    ...
    handleMenuItemClick(memuItem) {
      if (memuItem.subList.length <= 0) {
        ...
        this.activeGroup.activeSubItem = undefined;
        if(this.$route.path !== memuItem.path)
        this.$router.replace({
          path: memuItem.path,
        });
      }
  ...
  watch: {
    "activeGroup.activeSubItem": function (newSubItem, oldSubItem) {
      if (
        newSubItem !== undefined &&
        (oldSubItem === undefined || newSubItem.path !== oldSubItem.path)
      ) {
  ...
  .text_menu_item {
    ...
    &:hover {
      ...
      .text_menu_sub_item_panel {
        visibility: visible;
        -webkit-animation: menu_down 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)
          both;
        animation: menu_down 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
      }
    ...
    .text_menu_text {
      ...
      .rotate180 {
        animation: rotate180 0.5s ease-out 0s;
        animation-fill-mode: both;
      }

      .rotate0 {
        animation: rotate0 0.5s ease-out 0s;
        animation-fill-mode: both;
      }
