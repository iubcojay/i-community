...
export default {
  ...
  created() {
    EventBus.$on('triggerTimelineMethod', () => {
  ...
  methods: {
    ...
    updateCurrentTimeStamp() {
      if (this.playing) {
        ...
        // console.log("进度条：",this.currentTimeStamp);
        // if(this.currentTime <= 1720182099000){
        this.updateCurrentTime(this.currentTimeStamp);
        // }
  ...
  beforeDestroy() {
    EventBus.$off('triggerTimelineMethod', () => {
      console.log("offplayClick")
      this.handlePlayClick();
    });
  },
  destroyed() {
