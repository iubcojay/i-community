<template>
  <div class="accident_scene_container">
    ...
    <div class="time_line_container" v-show="false">
      <time-line-component ref="timeLine"
        v-if="timelineShow"
...
<script>
import TransportComponent from "@/views/index/community/internet/monitor/common/fireTruckSaveMe/TransportComponentCCC";
import FireTruckComponent from "@/views/index/community/internet/monitor/common/fireTruckSaveMe/FireTruckComponentCCC";
import TimeLineComponent from "@/views/index/community/internet/monitor/common/fireTruckSaveMe/TimeLineComponentCCC";
export default {
  ...
  data() {
    return {
      // transportModelUrl:
      //   this.$StaticResourceAddress + "/model/garage/car/suv.glb",
      // fireTruckModelUrl:
  ...
  methods: {
    // callFireTruck(){
    //   this.$refs.timeLine.handlePlayClick();
    // },
    resetAccidentScene() {
      this.clearScene();
      this.setClock();
    ...
    updateTransportInfo(entity, timestamp) {
      ...
      if (entity.status === "running") {
        ...
        accidentInfo.sentimentList.push({
          timestamp,
          values: entity.sentiments,
        });
      } else {
    ...
    updateFireTruckInfo(entity, timestamp) {
      ...
      if (entity.status === "to") {
        ...
        if (rescueInfo.interval === undefined) {
          rescueInfo.interval = [fireTruckInfo["to"].interval[1], timestamp];
        } else {
