<template>
  <div>
    <FolderPanelComponent title="调研计划" position="right">
      ...
      <template slot="content">
        <div class="content_panel">
          ...
          <el-input
            ...
            :rows="8"
            placeholder="请填写调研计划"
            v-model="planInformation"
            style="font-size: 18px;"
          ></el-input>
          <div class="button_panel">
            <el-button type="primary" plain @click="handleReceiveTaskBtnClick">接受任务</el-button>
          </div>
        </div>
      </template>
...
export default {
  name: "ProcessGridReceiveRightComponent",
  ...
  methods: {
    handleReceiveTaskBtnClick() {
      if (this.planInformation===""){
          this.$message.error("接受任务前请先填写调研计划")
      }else {
          this.supMethods.receiveTask(this.selectedTask.id,this.userInfo.id,this.planInformation)
