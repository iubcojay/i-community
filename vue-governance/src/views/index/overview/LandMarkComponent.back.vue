...
class Road {
  ...
  initEntities() {
    this.roadInfoList.forEach((roadInfo) => {
      const cartesian3PositionArray = Cesium.Cartesian3.fromDegreesArray(
        roadInfo.geometry.coordinates.flat()
      );
  ...
  addTo(entities) {
    this._entityCollection.values.forEach((entity) => entities.add(entity));
  }

  removeFrom = (entities) => {
    this._entityCollection.values.forEach((entity) => entities.remove(entity));
  };
  ...
  methods: {
    ...
    async initRoads() {
      const { data } = await this.$axios.get(
        this.$StaticResourceAddress + "/json/road/roads.json"
      );
      const roadInfoList = data.features;

      // 按照道路名称分类
      const roadInfoMap = _.groupBy(
        roadInfoList,
        (roadInfo) => roadInfo.properties.name
      );

      const entries = Object.entries(roadInfoMap);
      for (const [key, value] of entries) {
        // 单独处理无名道路
        if (key.trim() === "") {
          continue;
        }
        const road = new Road(key, value);
    ...
    clearRoads() {
      ...
      // const worker = ClearRoadWorker();
      // worker.postMessage({
      //   message: "start",
      //   roadEntityIdList: this.roadList.flatMap((road) =>
      //     road._entityCollection.values.map((entity) => entity.id)
      //   ),
      // });
